---
name: akshare
description: AKShare 金融数据库接口的检索与执行。AKShare 提供 1000 余个股票、基金、债券、期货、宏观等数据接口，当用户需要从自然语言描述定位接口（"查可转债实时行情"、"股票历史行情怎么取"）、确认某个接口的参数与返回字段、或实际调用接口获取金融数据时使用本技能。也支持一键运行现成分析：当日 A 股市场资金流向报告、基金持仓分析报告（持仓集中度、行业配置、重仓股新闻情绪、操作建议）、板块轮动分析（近 1 月资金流 + 当日快讯 + 行业财报三维打分，输出值得投资/建议回避的板块与基金推荐）。典型场景：写金融数据采集代码前先检索接口，执行接口取数并落盘为 CSV/JSON；或直接问"帮我分析基金 000001 的持仓"、"现在哪些板块值得投资"。
---
...
## 脚本四：基金持仓分析 fund_analysis.py

输入一个或多个基金代码，自动抓取最新季度持仓、行业配置、重仓股近期新闻与当日行业资金流，内置规则引擎给出分析结论与操作建议：

```bash
uv run python fund_analysis.py 000001
uv run python fund_analysis.py 000001 110011 519736   # 多基金
uv run python fund_analysis.py 000001 --top-n 5       # 抓新闻的重仓股数量（默认 10）
uv run python fund_analysis.py 000001 --year 2025     # 行业配置起始年份（默认当前年份，无数据自动前溯）
uv run python fund_analysis.py 000001 --report-only   # 仅数据报告，不做分析建议
uv run python fund_analysis.py 000001 --json          # 结构化输出
```
...
| 板块 | 数据源 | 内容 |
|---|---|---|
| TOP10 重仓股 | 天天基金（`fund_portfolio_hold_em`） | 代码/名称/占净值比例/持股数/市值，最新季度持仓 |
| 行业配置 | 天天基金（`fund_portfolio_industry_allocation_em`） | 证监会行业大类的占比与市值，最新截止时间 |
| 重仓股新闻情绪 | 东方财富（`stock_news_em`） | 按关键词分正面/负面/中性，展示代表性新闻 |
| 分析评估 | 综合计算 | 持仓集中度（前5大占比评级）、行业配置对照当日同花顺资金流 |
| 操作建议 | 规则引擎 | 集中度/行业集中度/资金流出/新闻风险提示 + 一句话结论（建议持有/关注/谨慎） |

**要点：**

- 持仓与行业配置各接口独立重试（默认 4 次、间隔 3 秒）；新闻并发抓取（5 线程），单只失败不阻塞整体。
- 新闻情绪仅基于标题关键词判断，正负同现时按负面优先；标题不含股票名或属板块级新闻归为中性——结果是启发式的，仅供参考。
- 行业配置为证监会大类，通过关键词映射到同花顺细分行业匹配资金流，无法映射的大类（如制造业仅匹配主流细分）显示"无细分资金流数据"。
- 行业资金流为当日即时数据，持仓为最新季报——两者时间窗口不同，资金流反映的是短端情绪而非当季配置动向。

## 脚本五：板块轮动分析 sector_rotation.py

基于近 1 个月的行业资金流、当日重大财经快讯与行业龙头财报，对同花顺 90 个行业板块做三维打分，输出值得投资 / 建议回避的板块，并给出对应的基金购入参考：

```bash
uv run python sector_rotation.py                        # 默认关注 TOP10 + 回避 TOP5
uv run python sector_rotation.py --top-n 5 --bottom-n 3 # 自定义榜单数量
uv run python sector_rotation.py --quarter 20260630     # 指定财报期（默认自动推断最近披露期）
uv run python sector_rotation.py --json                 # 结构化输出
```

评分构成（总分 100）：20 日资金分位 × 40 + 当日资金分位 × 10 + 新闻情绪 20（基准 10 分，每条净正面 +2）+ 财报基本面 30（行业净利增速中位数线性映射：-50%→0 分、0%→15 分、≥+50%→满分）。≥65 重点关注、≥45 中性观望、其余建议回避。

| 数据 | 数据源 | 用途 |
|---|---|---|
| 近 20 日行业资金流 | 同花顺（`stock_fund_flow_industry` symbol="20日排行"） | 资金主力维度（40%） |
| 当日行业资金流 | 同花顺（`stock_fund_flow_industry` symbol="即时"） | 当日验证（10%） |
| 当日财经快讯 | 东方财富（`stock_info_global_em`，200 条） | 政策头条区 + 板块情绪打分 |
| 行业财报 | 东方财富（`stock_yjbb_em`） | 净利/营收增速中位数、龙头个股业绩 |
| 基金推荐 | 东方财富（`fund_open_fund_rank_em` / `fund_exchange_rank_em`） | 按板块主题关键词匹配近端动量领先基金 |

**要点：**

- 财报期自动推断：最近季度末披露量不足 300 家时逐季前溯，避免用披露初期的残缺数据打分。
- 东财行业名（带 Ⅱ/Ⅲ 后缀）自动匹配同花顺板块名；无法匹配的板块财报维度按中性 15 分。
- 基金推荐优先近 3 月收益为正的品种（动量分 = 近3月 + 0.5×近1月），同一基金 A/C 份额去重；每板块各推荐 2 只场外基金与 2 只 ETF。
- 新闻情绪仅基于当日 200 条快讯的关键词匹配，是短端情绪信号而非中期趋势判断。
- 该脚本的 4 个数据接口从 akshare 子模块直接导入（`akshare/__init__.py` 过大导致 pyright 部分符号解析失效），pyproject 中已关闭 `reportMissingTypeStubs`。
